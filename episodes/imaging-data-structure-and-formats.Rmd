...
example data sets come from the organisers and Chris Rorden's 
...
If you list the contents, you will see three separate subfolders:
```bash
ls
```
```output
dcm_qa_xa30  dcm_qa_xa30i  Subject01
```
...
```
```output
Chris Rorden's dcm2niiX version v1.0.20260416  GCC14.3.0 x86-64 (64-bit Linux)
usage: dcm2niix [options] <in_folder>
 Options :
  -1..-9 : gz compression level (1=fastest..9=smallest, default 6)
  -a : adjacent DICOMs (images from same series always in same folder) for faster conversion (n/y, default n)
...
::::::::::::::::::::challenge
*Construct a command to convert the images in the*
`Subject01` *directory, storing the output in the directory*
...
Convert 196 DICOM as Nifti/Subject01_GenerationR-Adults-Neuro_20210105173554_300 (220x220x196x1)
Convert 64 DICOM as Nifti/Subject01_GenerationR-Adults-Neuro_20210105173554_400 (512x512x64x1)
Conversion required 3.165409 seconds (0.639208 for core code).
```
::::::::::::::::::::::
::::::::::::::::::::::
...
::::::::::::::: spoiler
```output
Subject01_GenerationR-Adults-Neuro_20210105173554_300.json
Subject01_GenerationR-Adults-Neuro_20210105173554_300.nii
Subject01_GenerationR-Adults-Neuro_20210105173554_400.json
Subject01_GenerationR-Adults-Neuro_20210105173554_400.nii
```

::::::::::::::::::::::::

You will see files that have the same name, but with different extensions:
...
- `.bval` and `.bvec` extensions: You may see these additional files if you are
converting [diffusion imaging](diffusion-weighted-imaging.Rmd), as they are 
needed to store gradient information.
...
(for example: `Subject01_GenerationR-Adults-Neuro_20210105173554_400.nii`). Now these
NIfTI files have a `.gz` at the end --> `Subject01_GenerationR-Adults-Neuro_20210105173554_400.nii.gz`
...
- `%f` is replaced by the folder name `Subject01`
- `%p` is replaced by the protocol `GenerationR-Adults-Neuro`
- `%t` is replaced by the date and time of the acquisition `20210105173554`
- `%s` is replaced by the series number, `300` or `400`
...
`Subject01_GenerationR-Adults-Neuro_20210105173554_400.nii.gz` is now named
`genrparent_test_400_PU_Ax_T2_FLAIR.nii.gz`
:::::::::::::::::::::
:::::::::::::::::::::

::::::::::::::::::::challenge
*Try converting the data found in some of the additional directories, `dcm_qa_xa30`. 
What sequences does it look like this acquisition contains?*

::::::::::::::::::::: hint
Make sure to include the Series description or the prtocol flag  to find the 
different names of the data. 

**NOTE** If you see `Hucheng` in the output filenames, do not worry, 
as this is the name of the individual who acquiring the data and provided
...
```bash
dcm2niix -o Nifti -f %i_%s_%d dcm_qa_xa30
```
and then looking at the output text from the command and the files 
found in the `Nifti` directory, we see the following scans:

* Structural T1-weighted images (`12343@hucheng_5_anat-T1w_acq-tfl.nii`)
* A functional MRI scan (`12343@hucheng_7_func-bold_task-fa_run-1.nii`)
* DWI images (`12343@hucheng_10_DWI_dir80_PA.nii` with `.bvec` and
`.bval extensions as well)
* A field mapping scan, which helps with DWI and fMRI processing (`12343@hucheng_8_gre_field_mapping_e1.nii` for example, but there are multiple echoes)
* ASL images (`12343@hucheng_25_asl_2d_tra.nii`)
